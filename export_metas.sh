...
# Evita concorrência
exec 9> /opt/relatorio_h_t/.export_metas.lock
flock -n 9 || { echo "$(date -Is) já existe execução em andamento" >> logs/export_metas.log; exit 0; }
...
# (se sync_www.sh já copia json_metas, ótimo; senão, ajuste no script)
