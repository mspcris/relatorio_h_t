...
SQL_FULL_DIR    = os.path.join(BASE_DIR, "sql_full_rateio")
DADOS_FULL_DIR  = os.path.join(BASE_DIR, "dados_rateio")
JSON_FULL_DIR   = os.path.join(BASE_DIR, "json_rateio")
...
def collect_full_sql_files():
    ...
    """
    files = sorted(glob.glob(os.path.join(SQL_FULL_DIR, "fin_*.sql")))
    items = []
    for f in files:
...
def build_json_full(key: str):
    ...
    for fn in os.listdir(DADOS_FULL_DIR):
        ...
        # 👇 AQUI — filtro que remove o warning do pandas
        if df.empty:
            continue
    ...
    for _, row in big.iterrows():
        ...
        # 🔒 JSON-safe: troca NaN / inf por null
...
def run():
    ...
    if args.only_month:
        ...
        DEFAULT_RATEIO_START = ym_to_date("2025-12")

        start = (
            ym_to_date(args.from_ym)
            if args.from_ym
            else DEFAULT_RATEIO_START
    )
    ...
    print("\n[JSON] Consolidação rateio - fin_despesa_plano")
    build_json_full("fin_despesa_plano")

    print("\n[JSON] Consolidação rateio - fin_despesa_planodeprincipal")
    build_json_full("fin_despesa_planodeprincipal")

    print("\n[JSON] Consolidação rateio - fin_despesa_tipo")
